...
# NEW
GAP_PX = 120   # try 0, 20, 40, 80, 120, 160 ...

# move red right until its right edge meets green's left edge (true contact)
elapsed_red = 0
...
# INSERT THE SPATIAL GAP INSTANTLY (no time delay)
gx, gy = s2.position
s2.position = (gx + GAP_PX, gy) 

# redraw once so the gap is visible immediately
s.present(clear=True,  update=False)
s2.present(clear=False, update=True)
